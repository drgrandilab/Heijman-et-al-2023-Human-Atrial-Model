...
subplot(2,3,2),hold on,plot(HR(HR_low:HR_up),outputs(HR_low:HR_up,5),'Color',colorarray)
set(gca,'box','off','tickdir','out','fontsize',12)
ylabel('APD90 (ms)')%, xlabel('HR (Hz)')
subplot(2,3,1),hold on,plot(HR(HR_low:HR_up),outputs(HR_low:HR_up,7),'Color',colorarray)
set(gca,'box','off','tickdir','out','fontsize',12)
ylabel('APD50 (ms)')%, xlabel('HR (Hz)')
subplot(2,3,4),hold on,plot(HR(HR_low:HR_up),-outputs(HR_low:HR_up,3),'Color',colorarray)
set(gca,'box','off','tickdir','out','fontsize',12)
ylabel('Em rest (mV)'), xlabel('HR (Hz)')
subplot(2,3,5),hold on,plot(HR(HR_low:HR_up),outputs(HR_low:HR_up,11)*1e6,'Color',colorarray)
set(gca,'box','off','tickdir','out','fontsize',12)
ylabel('CaT amp (nM)'), xlabel('HR (Hz)')
...
bar([66 25 9;outputs(2,18) outputs(2,19) outputs(2,20)]),ylim([0 100])
errorbar([0.78 1 1.22],[66 25 9],[7 6 2],'k.')
set(gca,'box','off','tickdir','out','fontsize',12)
title('nSR @ 0.5 Hz')
ylabel('% contribution')
legend('SERCA','NCX','PMCA')
set(gca, 'XTick', 1:2, 'XTickLabel', {'Experiments','Simulations'})
...
APD90_sim_x = HR(HR_low:HR_up);
APD90_sim_y_nSR = outputs(HR_low:HR_up,5);

%% AF
if AF_flag == 0
    Fig_S6_Freq_analysis_AF
    APD90_sim_y_cAF = outputs(HR_low:HR_up,5);
end

%% Exp data
figure(3),subplot(4,5,1) % UV
plot([0.5 0.5],[200 240],'*k--')
figure(3),subplot(4,5,5) % APD90
plot([0.2 3],[260 180],'*k--')
plot([0.2 3],[120 90],'*r--')
subplot(4,5,10) % CaTmin
plot([0.5],[160],'*k--')
plot([0.5],[200],'*r--')
subplot(4,5,11) % CaTamp
plot([0.5],[220],'*k--')
plot([0.5],[100],'*r--')
subplot(4,5,15) % Na
plot([3],[14],'*k--')
subplot(4,5,16) % CaSRmax
plot([0.5 0.5],[0.4 0.5],'*k--')
subplot(4,5,18) % B3 serca
plot([0.5],[66],'*k--')
plot([0.5],[46],'*r--')
subplot(4,5,19) % B3 ncx
plot([0.5],[25],'*k--')
plot([0.5],[45],'*r--')
